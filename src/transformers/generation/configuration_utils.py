...
class GenerationConfig(PushToHubMixin):
    ...
    def validate(self, strict=False, user_set_attributes: set[str] | None = None):
        ...
        if self.do_sample is not True:
            ...
            if self.p_less is not None and _should_warn("do_sample", "p_less", user_set_attributes):
                minor_issues["p_less"] = greedy_wrong_parameter_msg.format(flag_name="p_less", flag_value=self.p_less)
            if self.p_less_norm is not None and _should_warn("do_sample", "p_less_norm", user_set_attributes):
