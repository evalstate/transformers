...
def grouped_mm_experts_forward(
    ...
    expert_ids_g_for_bias = None
    if self.has_bias:
        # Clamp for the per-row bias gather below to stay in-bounds, while keeping
        # expert_ids_g unchanged so sentinel rows can still be detected and zeroed later.
        expert_ids_g_for_bias = expert_ids_g.clamp(0, self.num_experts - 1)
    ...
    if self.has_gate:
        ...
        selected_biases = _local(self.gate_up_proj_bias)[expert_ids_g_for_bias] if self.has_bias else None
    else:
        selected_weights = _local(self.up_proj)
        selected_biases = _local(self.up_proj_bias)[expert_ids_g_for_bias] if self.has_bias else None
    ...
    selected_biases = _local(self.down_proj_bias)[expert_ids_g_for_bias] if self.has_bias else None
