    ...
    * ``prefetch_depth=N``: *N* rollouts are queued.  Useful when
    ...
    def produce(self, model: Any, global_step: int, **kwargs) -> Dataset:
        ...
        if self._warmup_remaining > 0:
            ...
            logger.info(f"AsyncDataProducer: sync warmup rollout (remaining={self._warmup_remaining})")
        ...
        if not self._initialized:
            ...
            for i in range(1, self._depth + 1):
                self._queue.append(self._executor.submit(self._inner.produce, model, global_step + i, **bg_kwargs))
        ...
        self._queue.append(self._executor.submit(self._inner.produce, model, next_step, **bg_kwargs))
        return dataset
