...
class _TimestampOverlayCache:
    ...
    def apply(self, image: torch.Tensor, timestamp: str, size_factor: float = 0.1) -> torch.Tensor:
        ...
        font_size = int(min(height, width) * size_factor)
        outline_size = int(font_size * size_factor)

        overlay, overlay_width, overlay_height = self._render_overlay(timestamp, font_size, outline_size)
        paste_height, paste_width = min(overlay_height, height), min(overlay_width, width)

        result = image.clone()
        alpha = overlay[3:4, :paste_height, :paste_width].float() / 255.0
        rgb_overlay = overlay[:3, :paste_height, :paste_width].float()
        original_region = result[:, :paste_height, :paste_width].float()
        blended = alpha * rgb_overlay + (1.0 - alpha) * original_region
        result[:, :paste_height, :paste_width] = blended.to(result.dtype)
