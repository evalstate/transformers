...
class MixtralExperts(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            expert_hit = torch.greater(expert_mask.sum(dim=(-1, -2)), 0).nonzero() if self.training else None

        expert_indices = expert_hit if self.training else range(self.num_experts)
