...
@auto_docstring
class UMT5EncoderForSequenceClassification(UMT5PreTrainedModel):
    keys_to_ignore_on_load_unexpected = [r"decoder"]

    # Copied from transformers.models.t5.modeling_t5.T5EncoderForSequenceClassification.__init__ with T5->UMT5
    def __init__(self, config: UMT5Config):
        super().__init__(config)
        ...
        self.transformer = UMT5EncoderModel(config)
        self.dropout = nn.Dropout(config.classifier_dropout)
        self.classifier = UMT5ClassificationHead(config)
