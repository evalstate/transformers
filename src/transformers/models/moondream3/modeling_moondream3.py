...
class Moondream3RotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(
        config: Moondream3Config | None = None,
        ...
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.float32)[: (dim // 2)] / dim))
    ...
    def forward(self, x, position_ids):
        inv_freq_expanded = (
            self.inv_freq[None, :, None].to(torch.float32).expand(position_ids.shape[0], -1, 1).to(x.device)
        )
...
class Moondream3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_moe_layer:
            hidden_states_mlp, router_logits = self.mlp(hidden_states_ln, cache_position=cache_position)
        else:
...
class Moondream3VisionModel(Moondream3PreTrainedModel):
    ...
    def __init__(self, config: Moondream3VisionConfig):
        ...
        self.layers = nn.ModuleList(
            [Moondream3VisionEncoderLayer(config, layer_idx) for layer_idx in range(self.num_hidden_layers)]
        )
...
class Moondream3GenerateOutput(GenerateDecoderOnlyOutput):
    objects: list[dict[str, float]] | None = None
...
class Moondream3ForConditionalGeneration(Moondream3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        mask = torch.ones(hidden_states.shape[0], 1, device=self.device, dtype=torch.long)
        ...
        while is_processing_point:
            ...
            if input_ids.shape[-1] > 1 and input_ids[0, 1] == 7235:
                ...
                next_embeds = (
                    self.model.region_encoder.encode_size(torch.stack([w, h], dim=-1).to(size_logits.dtype))
                ).unsqueeze(1)
                bbox = [
    ...
    def generate(self, **kwargs) -> Moondream3GenerateOutput | torch.LongTensor:
