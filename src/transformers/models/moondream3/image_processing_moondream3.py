...
def overlap_crop_image(
    ...
    # Create global crop
    global_pil = pil_img.resize((int(base_size[1]), int(base_size[0])), resample=PIL.Image.Resampling.LANCZOS)
...
def prepare_crops(image, max_crops=12, overlap_margin=4):
    ...
    all_crops = all_crops = (
        torch.from_numpy(all_crops).to(device="cpu", dtype=torch.bfloat16).div_(255.0).sub_(0.5).div_(0.5)
    )
