...
class Trainer:
    ...
    def _get_signature_columns(self) -> list[str]:
        """Return dataset columns accepted by the model's forward method."""
    ...
    def train(
        ...
        if resume_from_checkpoint is not None:
            ...
            if os.path.isfile(trainer_state_path):
                ...
                logger.warning(
                    f"TrainerState file not found at {trainer_state_path}. "
    ...
    def _log_training_banner(
        self, plan, epochs_trained, steps_trained_in_current_epoch, model, resume_from_checkpoint
    ):
        ...
        logger.info(
            f"  Total train batch size (w. parallel, distributed & accumulation) = {plan.total_train_batch_size:,}"
        )
    ...
    def _inner_training_loop(
        ...
        epochs_trained, steps_trained_in_current_epoch = self._init_training_state(
            plan.max_steps,
            plan.num_update_steps_per_epoch,
            plan.num_train_epochs,
            resume_from_checkpoint,
            trial,
        )
        model, train_dataloader = self._prepare_for_training(
            plan.max_steps,
            source.initial_dataloader,
            resume_from_checkpoint,
        )
    ...
    def save_model(self, output_dir: str | None = None, _internal_call: bool = False) -> None:
        ...
        if is_torch_xla_available():
            save_tpu_checkpoint(
                ...
                output_dir,
                is_fsdp_xla_v2_enabled=self.is_fsdp_xla_v2_enabled,
            )
        elif is_sagemaker_mp_enabled():
