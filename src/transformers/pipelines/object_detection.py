...
class ObjectDetectionPipeline(Pipeline):
    ...
    def postprocess(
        ...
        if self.tokenizer is not None:
            ...
            annotation = [
                dict(zip(keys, vals)) for vals in zip(scores.tolist(), label_names, boxes) if vals[0] > threshold
            ]
        else:
    ...
    def _get_bounding_box(
        ...
        if box_format == "xyxy":
            ...
            raise ValueError(f"Invalid `box_format` '{box_format}'. Choose one of 'xyxy', 'xywh', or 'normalized'.")
