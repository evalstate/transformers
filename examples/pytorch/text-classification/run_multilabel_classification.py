...
def binarize_probs(p: np.ndarray, th: float) -> np.ndarray:
    return (p >= th).astype(np.int64)
...
def main():
    ...
    if args.do_eval and args.validation_split not in raw:
        raise ValueError("missing validation split")
    ...
    def maybe_select(ds, n):
        return ds if n is None else ds.select(range(min(n, len(ds))))
    ...
    if args.do_eval:
        ...
        if args.tune_thresholds:
            ...
            f1s = []
            best = {"threshold": base_threshold, "f1_micro": -1.0, "metrics": None}
            for th in ths:
            ...
            if args.plot_threshold_curve and plt is not None:
                ...
                plt.ylabel("F1-micro")
                plt.title("Validation F1-micro vs Threshold")
